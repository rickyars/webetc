/**
 * Test Keccak-512 for DAG generation compatibility
 */

import sha3Lib from 'js-sha3';
const sha3 = sha3Lib;

// Copy keccak512 implementation from standalone.html
...
function keccak512(message) {
    const OUTPUT_SIZE = 64;
    const BLOCK_SIZE = (1600 - (512 << 1)) >> 5; // 18 words
    const BYTE_COUNT = BLOCK_SIZE << 2; // 72 bytes
...
// Test
console.log('=== Keccak-512 Test ===\n');

const testInputs = ['', 'abc'];
for (const input of testInputs) {
  console.log(`\nTesting: "${input}"`);
  const ourResult = bytesToHex(keccak512(new TextEncoder().encode(input)));
  const refResult = sha3.keccak512(input);

  console.log(`Our result: ${ourResult}`);
